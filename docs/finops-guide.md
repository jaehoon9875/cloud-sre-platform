...
cost-reporter.py가 매일 Slack으로 비용 리포트를 전송하기 위한 Webhook 설정.  
**최초 1회만 실행**하면 된다.

### Step 1. Slack App 생성

1. [https://api.slack.com/apps](https://api.slack.com/apps) 접속 → **Create New App** 클릭
2. `From scratch` 선택 → 앱 이름 입력 (`gcp-cost-reporter`) → 워크스페이스 선택 → **Create App**

### Step 2. Incoming Webhook 활성화

1. 좌측 메뉴 **Incoming Webhooks** 클릭
2. 상단 토글 **Activate Incoming Webhooks** → ON
...
| Secret 이름 | 값 |
|-------------|-----|
| `SLACK_WEBHOOK_URL` | Step 2에서 복사한 Webhook URL |

> Webhook URL은 외부에 노출되지 않도록 주의한다. 코드나 로그에 직접 출력하지 않는다.
